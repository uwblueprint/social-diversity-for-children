...
import { Stack, Box, HStack, Text, Button, VStack } from "@chakra-ui/react";
...
export const ProofOfIncomeWrapper: React.FC<ParentEnrolledPageProps> = ({
    hasProofOfIncome,
    isLowIncome,
    pageNum,
    onNext,
    ...
    const [updateProofOfIncome, setUpdateProofOfIncome] =
        useState<boolean>(false);
    if (hasProofOfIncome === null || updateProofOfIncome) {
        return (
            <>
                ...
                <Box>
                    <HStack spacing="24px">
                        <FormButton
                            onClick={() => {
                                router
                                    .push("/document-upload?type=income-proof")
        ...
        return (
            <DiscountPage
                onNext={onNext}
                setUpdateProofOfIncome={setUpdateProofOfIncome}
            />
        );
    } else {
        return <>{pageNum == 5 && onNext()}</>;
    }
};
